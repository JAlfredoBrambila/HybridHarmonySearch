 ...
 */
public class HybridHarmonySearch implements SingleobjectiveAlgorithm<Individual> {
    // Problem Definition
    int nVar;
    double[] varMin;
    double[] varMax;
    ...
    public HybridHarmonySearch(Problem problem, int maxIt, int hms, int nNew, double hmcr, double par, double cFW, double FW_damp) {
        ...
        algoritm1 = new PSOIOBL(
                    this.problem,// Problem
                    50, // Maximum Number of Iterations
                    5, // Pop Size 100
                    0.7298, // W
                    0.99, // W Damp 1.0   0.99
                    1.49618, // C1 1.5    1.49618
                    1.49618, // C2 2.0    1.49618
    ...
    public HybridHarmonySearch(Problem problem, int maxIt, int hms, int nNew, double hmcr, double par, double cFW, double FW_damp, boolean experimentation) {
        this.problem = problem;
        // Problem Definition
        nVar = problem.getnVars();
        varMin = problem.getLowerBound();
        ...
        algoritm1 = new PSOIOBL(
                    this.problem,// Problem
                    50, // Maximum Number of Iterations 30
                    5, // Pop Size 100                 20
                    0.7298, // W
                    0.99, // W Damp 1.0   0.99
                    1.49618, // C1 1.5    1.49618
...
//        FW_damp = 0.995; // Fret Width Damp Ratio
    ...
    public void execute() {
        ...
        generateHarmonies();
        this.besCost = HM.get(0).getCost()[0];
            ...
            */
            ...
            if(it>0) {
                ...
                if(stagnation >= (int)(MaxIt * this.sigma)) {
                    ...
                    //Sort
                    DoubleUtils.sortHMListByCost(HM);
                } /*else if(Math.random() > this.sigma) {
                    restartHarmonies(); //////
                }*/
            }
            ...
            //if(this.besCost != HM.get(0).getCost_0()) {
            if(HM.get(0).getCost_0()<this.besCost ) {
                this.timeReg = System.currentTimeMillis() - initialTime;
                //System.out.println("Set: " + this.besCost + " n: " + HM.get(0).getCost_0()+" IT: " + it);
                this.besCost = HM.get(0).getCost_0();
                //System.out.println("Time: " + this.timeReg);
                //System.out.println("Time: " + this.timeReg);
            }
        ...
        ////
        if (EXPERIMENTATION) {
            DoubleUtils.createDirectory(Paths.get("").toAbsolutePath().toString() + "\\exp\\HS_PSOB\\");
            //DoubleUtils.createDirectory(Paths.get("").toAbsolutePath().toString() + "\\fronts\\DNSGA\\" + problem.getProblemName() + "\\F" + contW);
            //
            DoubleUtils.saveIndividualToFileNTimeAppend(bestSol, this.timeReg, "exp\\HS_PSOB\\" + problem.getProblemName() + "_.csv");
            //DoubleUtils.saveIndividualToFileNTimeAppend(bestSol, (endTime - initialTime), "exp\\HS_PSOB\\" + problem.getProblemName() + "_.csv");
    ...
    public void restartHarmonies() {
        ...
        for(int i=0; i<nRegen; i++) {
            ...
            Particle particle = algoritm1.getBestParticle(this.HM);
            //genera valores reales aletatorios(entre varMin y varMax) para el cromosoma de tamaño nVar
            ind.setPosition(particle.getPosition());
           ...
           if(ind.getCost()[0] < HM.get(index).getCost()[0]) {
               //System.out.println("C: " + ind.getCost()[0]);
