 ...
 */
public class F13 implements Problem {
    ...
    public void costFunction(Individual ind) {
        ...
        sum = (0.1 * Math.pow(Math.sin(3*Math.PI * ind.getPosition()[0]), 2));
        double sumU = 0.0;
        for(int i=0; i<ind.getPosition().length; i++) {
            //f[0] += Math.floor(Math.pow((ind.getPosition()[i]), 2));
            sumU += this.getU(ind.getPosition()[i], 5, 100, 4);
        }
        ...
        for(int i=0; i<ind.getPosition().length; i++) {
            ...
            sum2 += Math.pow((ind.getPosition()[i] - 1), 2) * (1 + Math.pow(Math.sin(3 * Math.PI * ind.getPosition()[i]), 2));
        }
        
        f[0] = sum + sum2 + + Math.pow((ind.getPosition()[ind.getPosition().length-1]), 2) * 1 + Math.pow(Math.sin(2 * Math.PI * ind.getPosition()[ind.getPosition().length-1]), 2) + sumU;
