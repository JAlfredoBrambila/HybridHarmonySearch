 ...
 */
public class DiferentialEvolution4 implements CrossoverOperator<Individual> {
    ...
    public DiferentialEvolution4() {
        
    }
    
    public DiferentialEvolution4(double[] lower, double[] upper) {
        this.lo = lower;
        this.up = upper;
    }
    ...
    public Individual doCrossover(LinkedList<Individual> parents) {
        ...
        for(int j=0; j<nVars; j++) {
            if(DoubleUtils.getRandomNumber0_1() < CR || j == k) {
                offspring.getPosition()[j] = this.current.getPosition()[j] + K * (this.current.getPosition()[j]-parents.get(0).getPosition()[j]) + F * (parents.get(1).getPosition()[j]-parents.get(2).getPosition()[j]);
                //offspring.getPosition()[j] = parents.get(0).getPosition()[j] + K * (parents.get(0).getPosition()[j]-parents.get(1).getPosition()[j]) + F * (parents.get(1).getPosition()[j]-parents.get(2).getPosition()[j]);
            } /*else {
